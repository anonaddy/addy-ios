...
public extension Date {
    ...
    func aliasRowDateDisplay() -> String {
        ...
        if calendar.isDateInToday(self) {
            ...
            if diffSeconds == 0 {
                return NSLocalizedString("just_now", bundle: Bundle(for: SharedData.self), comment: "")
            } else if diffSeconds < 60 {
                return String(format: NSLocalizedString("d_sec_ago", bundle: Bundle(for: SharedData.self), comment: ""), diffSeconds)
            } else {
                let diffMinutes = calendar.dateComponents([.minute], from: self, to: currentDate).minute ?? 0
                if diffMinutes == 0 {
                    return String(format: NSLocalizedString("d_sec_ago", bundle: Bundle(for: SharedData.self), comment: ""), diffSeconds)
                } else if diffMinutes < 60 {
                    return String(format: NSLocalizedString("d_min_ago", bundle: Bundle(for: SharedData.self), comment: ""), diffMinutes)
                } else {
                    let diffHours = calendar.dateComponents([.hour], from: self, to: currentDate).hour ?? 0
                    if diffHours < 24 {
                        if diffHours == 1 {
                            return NSLocalizedString("1_hour_ago", bundle: Bundle(for: SharedData.self), comment: "")
                        } else {
                            return String(format: NSLocalizedString("d_hours_ago", bundle: Bundle(for: SharedData.self), comment: ""), diffHours)
                        }
                    } else {
                        let diffDays = calendar.dateComponents([.day], from: self, to: currentDate).day ?? 0
                        if diffDays < 30 {
                            if diffDays == 1 {
                                return NSLocalizedString("1_day_ago", bundle: Bundle(for: SharedData.self), comment: "")
                            } else {
                                return String(format: NSLocalizedString("d_days_ago", bundle: Bundle(for: SharedData.self), comment: ""), diffDays)
                            }
                        } else {
                            let diffMonths = calendar.dateComponents([.month], from: self, to: currentDate).month ?? 0
                            if diffMonths < 12 {
                                if diffMonths == 1 {
                                    return NSLocalizedString("1_month_ago", bundle: Bundle(for: SharedData.self), comment: "")
                                } else {
