    ...
    private func addQuickActions(alias: Aliases) {
        if !mainViewState.encryptedSettingsManager.getSettingsBool(key: .privacyMode) {
            ...
            UIApplication.shared.shortcutItems = [
                UIApplicationShortcutItem(type: "host.stjin.addy.shortcut_open_alias_\(alias.id)", localizedTitle: alias.email, localizedSubtitle: nil, icon: UIApplicationShortcutIcon(type: .time)),
            ]
        }
    }
